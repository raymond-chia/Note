---
name: bigquery
description: 查詢 BigQuery 資料的通用規則與流程（含費用紀律：先 dry run 再實查）。任何涉及 bq 指令的操作都觸發此 skill，包含查資料、看 schema、列 dataset/table，以及排程查詢（scheduled query／transfer config）的狀態與失敗排查。搭配 cli skill 處理 JSON 輸出。
user-invocable: true
---
...
使用者常直接貼 BigQuery Console 網址，例如：

```
https://console.cloud.google.com/bigquery?ws=!1m5!1m4!4m3!1sPROJECT!2sDATASET!3sTABLE
```

只需抓 `!1s` / `!2s` / `!3s` 這種欄位的值，依序對應：

- `!1s` = **project**、`!2s` = **dataset**、`!3s` = **table**。

## 執行流程

1. 確認使用者已提供日期（否則先問）。若輸入是 Console URL，先依上節解析出 project/dataset/table。
2. **若尚未確認 table schema**（沒讀過本地 schema 檔、或首次查該 table），先讀（免費）：
   ```
   bq show --project_id=<project> --format=json <project>:<dataset>.<table> | jq '{schema, timePartitioning, rangePartitioning, clustering}'
   ```
...
4. Dry run 估掃描量：
   ...
   ```
5. 告知使用者預估掃描量，**等確認**。
6. 實際執行：以 `--format=json` 直接存檔，同時寫出 SQL：
   ```
   SKILL="$(cd "$HOME/.claude/skills/bigquery" && pwd -P)"   # 先解析 symlink
   TMP="$(cd "$SKILL/../../tmp" && pwd -P)"
   D="$TMP/<日期>_<簡述>"   # 日期年份以 currentDate 為準
   printf '%s\n' 'SQL' > "$D.sql"
   bq query --use_legacy_sql=false --maximum_bytes_billed=17179869184 --max_rows=100000 --format=json 'SQL' > "$D.ndjson"
   ```
   `.ndjson` 為純結果（每行一物件），`.sql` 為當次 SQL。
   給人看的結果，從 `.ndjson` 用 jq 整理呈現即可。
7. 整理結果呈現給使用者。
...
- `bq ls` 預設只列 **50** 筆 tables，排序未定義（通常接近建立時間）。**別因只看到舊 table 就判斷 dataset 沒新資料。** 要確認是否仍有新資料，用足夠大的 `--max_results` 排序取末幾筆：
  ...
  ```
- `bq ls --transfer_run` 不帶 `--run_attempt` 時**每天只回最新一筆**（等同 `LATEST`，`--help` 未載明）。高頻排程會因此漏看失敗紀錄。要完整歷史須明確指定：
  ```
  bq --format=json ls --transfer_run --run_attempt=RUN_ATTEMPT_UNSPECIFIED \
    --max_results=1000 projects/<p>/locations/<loc>/transferConfigs/<id>
  ```
