...
1. 確認使用者提供了日期
2. **若尚未確認 table schema**（例如沒有讀過本地 schema 檔案、或首次查詢該 table），先執行：
   ```
   bq show --format=json <project>:<dataset>.<table> | jq '{schema, timePartitioning, rangePartitioning, clustering}'
   ```
3. 組合 SQL，確保包含正確的日期過濾與 `LIMIT`
4. `bq query --use_legacy_sql=false --dry_run 'SQL'` 確認掃描量
5. 告知使用者預估掃描量，確認後執行實際查詢
6. `bq query --use_legacy_sql=false 'SQL'`
7. 整理結果呈現給使用者
