...
執行任何 `glab` 指令時，必須依目前任務明確帶上目標 GitLab instance 的 `--hostname`。不得依賴預設 host，避免將 API 請求送到錯誤的 GitLab instance。
...
### Work item URL 對應 REST API

`/<group>/<project>/-/work_items/<iid>` 使用 issue API 查詢；project path 需 URL encode（`sre/issue` → `sre%2Fissue`）：
