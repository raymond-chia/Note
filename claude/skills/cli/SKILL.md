...
禁止自動執行費用不低的操作。執行前先用文字訊息輸出 `⚠️ 費用操作：<說明與預估成本>` 告知使用者，等待確認後才執行。典型例子：

- `bq query` 等實際掃描資料的 BigQuery query（依掃描量計費，費用不便宜）
- 觸發 Dataform workflow、scheduled query 等會跑運算的操作
- 任何按用量計費、可能產生顯著費用的雲端運算

相對地，**免費的 metadata 讀取不在此限**，可直接執行：`gcloud ... list`、`bq show` / `bq ls`、REST API 讀取 config、`glab api` 讀檔等。
