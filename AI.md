...
- Multi-Token Prediction
  - 主要 model 預測 token t+1
  - `draft` 拿主要 model 的 hidden state 預測多個 token (假設只有預測到 t+2)
    - 訓練的時候，就讓模型學會預測遠一點
  - 讓主要 model 驗證這一批 token t+1, t+2
    - 因為運算瓶頸是 memory 搬運權重，所以預測 `下一個` 跟 `驗證下兩個` 耗時差不多
    - 如果驗證成功，主要 model 就不用寫 t+2
    - 如果驗證失敗，回去讓主要 model 自己寫 t+2

#### Rank

- 使用量: https://openrouter.ai/rankings?view=month
- 成本 vs 效果: https://deepswe.datacurve.ai
...
- 擅長寫程式
- claude code 對話紀錄在 `~/.claude/projects`
- 設定環境變數來禁止使用 memory: CLAUDE_CODE_DISABLE_AUTO_MEMORY
- [投資](https://claude.com/solutions/financial-services#finance-agents)
...
- 提供工作流
- [annotation reply](https://docs.dify.ai/en/cloud/use-dify/monitor/annotation-reply): 先嘗試找人工寫的說明，找不到才用 LLM 試圖產生說明
...
- [NotebookLM](https://notebooklm.google.com): 整理上傳的檔案, 並提供問答
- rag
...
- score distillation sampling
  - https://www.youtube.com/watch?v=C1E500opYHA
  - 直接用 stable diffusion 產生 2d, 接著用 Gaussian splatting 或 NeRF 從多個 2d 算出 3d
  - Janus problem (因為每個面向的 stable diffusion 都以為自己才是正面)
  - 一開始用多 noise, 然後後續的其他面向逐漸降低 noise
    - 一開始會很糟, 但是慢慢變好 ??
- Tripo3D
