...
- redis-cli -n={db number} {command} | xargs redis-cli -n={db number} {command}
  - 使用 sentinel 時, port 要是 26379 ?? 可以用 6379 查詢 ??
- 設定
...
- 連線
  - `gcloud compute ssh --zone={zone} --project={project} {intermediate-instance.name} -- -N -L {port}:{dst-host}:{port}`
  - [iap](#IAP)
  ...
  - cluster autoscaler attempts to keep these managed instance group sizes balanced when `scaling up`
    ...
    - scale down 的時候直接砍 underutilized nodes
      - 也就是 scale down 的時候, `group sizes` 可能`分佈不均`
        - `kubectl get pods -l app=ls -l component=api -o json | jq -r '.items.[].status.conditions.[].message | select( . != null )'` ??
      - https://github.com/kubernetes-sigs/descheduler 也許有用 ??
- node 會隨機開在 region 裡面的任何 zone
  - neg 建立時, 可能只有處理當下有 node 的 zone
  - schedule 到其他 zone 時, 會自動建立 neg
    - 但是可能要補設定 load balancer ??
...
- 如果在 DNS 好之前就建立 SSL Certificate, Certificate 可能卡在 Provisioning state
...
- 以 backend instance group 或 network endpoint group 為單位
  - neg: 要在 kubernetes metadata annotation 設定  
    ...
    - 沒有 node 的 zone 不會建 NEG
      - 檢查 `gcloud compute network-endpoint-groups list`
    - terraform 建立 backend service 時, 需要包含所有 NEG
...
- 設定環境變數
  ...
  - secrets
    ...
    - base64
- readiness probe: 是否能服務
- liveness probe: 是否該重啟 ??
...
- `mongo "mongodb://{user}:{password}@{host}/{db}" --authenticationDatabase={auth db}`
...
- `terraform import {component} "{其他識別 infrastructure 的參數 ...}"` 接管已經存在的 infrastructure
  - 有可能是之前手動產生的
